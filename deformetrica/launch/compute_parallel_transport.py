...
from deformetrica.core.model_tools.deformations.geodesic import Geodesic
from deformetrica.core.observations.deformable_objects.deformable_multi_object import (
...
def compute_parallel_transport(
    ...
    if need_to_project_initial_momenta:
        ...
        projected_momenta = (
            torch.cholesky_solve(velocity, cholesky_kernel_matrix)
            .squeeze()
            .contiguous()
        )
...
def compute_pole_ladder(
    ...
    mid_cp, mid_mom = Exponential.rk4_step(
        deformation_kernel, control_points, initial_momenta, h / 2
    )
    ...
    # Now we transport!
    final_cp, transported_mom = geodesic.forward_exponential.pole_ladder_transport(
        initial_shoot
    )

    # write results:
    write_3D_array(
        transported_mom.detach().cpu().numpy(), output_dir, "transported_momenta.txt"
    )
...
def compute_volume_preserving_pole_ladder(
    ...
    mid_cp, mid_mom, mid_shape = geodesic.forward_exponential.rk4_step(
        shape, control_points, initial_momenta, h / 2
    )
