...
class KeopsKernel(AbstractKernel):
    def __init__(
        self, gpu_mode=default.gpu_mode, kernel_width=None, cuda_type=None, **kwargs
    ):
    ...
    def convolve(self, x, y, p, mode="gaussian"):
        if mode == "gaussian":
            ...
            assert (
                x[0].device == y[0].device == p.device
            ), "x, y and p must be on the same device"
            assert (
                x[1].device == y[1].device == p.device
            ), "x, y and p must be on the same device"
    ...
    def convolve_gradient(self, px, x, y=None, py=None, mode="gaussian"):
        ...
        assert (
            px.device == x.device == y.device == py.device
