...
from deformetrica.core.model_tools.deformations.spline import Spline
...
class SplineRegression(GeodesicRegression):
    ...
    def compute_log_likelihood(
        ...
        template_data, template_points, control_points, momenta, external_forces = (
    ...
    def compute_attachment_and_regularity(
        ...
        for j, (time, obj) in enumerate(zip(target_times, target_objects)):
            ...
            attachment -= self.target_weights[
                j
            ] * self.multi_object_attachment.compute_weighted_distance(
    ...
    def _fixed_effects_to_torch_tensors(self, with_grad, device="cpu"):
        ...
        external_forces = utilities.move_data(
            external_forces,
            dtype=self.tensor_scalar_type,
            requires_grad=(not self.freeze_external_forces and with_grad),
            device=device,
        )
                ...
                if compute_residuals:
                    residuals.append(
                        self.multi_object_attachment.compute_distances(
                            deformed_data,
                            self.template,
                            dataset.deformable_objects[0][j],
                        )
                    )
                    residuals_list = [
                        [
    ...
    def _write_model_parameters(self, output_dir):
        ...
        if not self.freeze_external_forces:
            write_3D_array(
                ...
                output_dir,
                self.name + "__EstimatedParameters__ExternalForces.txt",
            )
