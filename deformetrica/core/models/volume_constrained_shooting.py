...
from deformetrica.in_out.array_readers_and_writers import read_3D_array, write_2D_list
...
class VolumeConstrainedShooting(DeterministicAtlas):
    def __init__(
        ...
        **kwargs,
        ...
        self.fixed_effects["intercept"] = torch.tensor([1.0])
        self.fixed_effects["size_effect"] = torch.tensor([0.0])
    ...
    def compute_attachment_and_regularity(
        ...
        for i, target in enumerate(target_size):
            new_attachment, new_regularity = (
                self.deform_and_compute_attachment_and_regularity(
                    self.exponential,
                    template_points,
                    control_points[i],
                    momenta[i],
        ...
        return self.compute_gradients(
            ...
            intercept,
            size_effect,
            self.freeze_size_effect,
            with_grad,
        )

    def deform_and_compute_attachment_and_regularity(
        self,
        exponential,
        template_points,
        control_points,
        ...
        scaling = -scaling if scaling < 0.0 else scaling
        ...
        regularity = -self.regularization * (
            (1.0 - scaling[0]) ** 2 + min(0.0, scaling[0]) ** 2
        )
    ...
    def _fixed_effects_to_torch_tensors(self, with_grad, device="cpu"):
        ...
        if self.dense_mode:
            assert (
                ("landmark_points" in self.template.get_points().keys())
        ...
        intercept = utilities.move_data(
            intercept, device=device, dtype=self.tensor_scalar_type, requires_grad=True
        )
        ...
        for i, subject_id in enumerate(dataset["subject_ids"]):
            ...
            if compute_residuals:
                residuals.append(
                    [
                        (
                            self.volume(deformed_data["landmark_points"].detach())
                            - dataset["target"][i]
                        )
                        / 1000
                    ]
    ...
    def _write_model_parameters(self, output_dir):
        write_2D_list(
            np.stack([self.get_intercept(), self.get_size_effect()]),
            output_dir,
            "scaling_parameters.txt",
            mode="a",
        )
