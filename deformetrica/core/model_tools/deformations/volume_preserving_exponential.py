...
class Exponential:
    ...
    def update(self):
        ...
        for i in range(self.number_of_time_points - 1):
            new_cp, new_mom, new_x = step(
                landmark_points_t[i], self.control_points_t[i], self.momenta_t[i], dt
            )
    ...
    def pole_ladder_transport(self, initial_shoot, initial_time_point=0):
        ...
        for i in range(initial_time_point, self.number_of_time_points):
            ...
            shoot = self.rk4_step(
                main_geodesic[i], self.control_points_t[i], -mom, h, return_mom=False
            )
    ...
    def rk4_inverse(self, base, x, y, h):
        ...
        def loss_and_grad(mom):
            ...
            loss = torch.sum(
                (shoot.contiguous().view(-1) - target.contiguous().view(-1)) ** 2
            )
            loss.backward()
