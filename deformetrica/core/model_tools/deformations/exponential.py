...
import deformetrica.support.kernels as kernel_factory
from deformetrica.core import default
from deformetrica.in_out.array_readers_and_writers import write_2D_array, write_3D_array
...
class Exponential:
    ...
    def light_copy(self):
        light_copy = Exponential(
            ...
            use_rk2_for_flow=self.use_rk2_for_flow,
        )
        return light_copy
    ...
    def shoot(self):
        ...
        assert (
            len(self.initial_control_points) > 0
    ...
    def flow(self):
        ...
        if self.dense_mode:
            assert (
                "image_points" not in self.initial_template_points.keys()
        ...
        if "landmark_points" in self.initial_template_points.keys():
            ...
            for i in range(self.number_of_time_points - 1):
                ...
                if self.preserve_volume:
                    initial_shape = (
                        landmark_points[i].clone().detach().requires_grad_(True)
                    )
                    ...
                    d_pos -= (
                        torch.sum(grad_star * d_pos)
                        / torch.sum(grad * grad_star)
                        * grad
                    )
        ...
        if "landmark_points" in self.initial_template_points.keys():
            ...
            for i in range(self.number_of_time_points - 1):
                ...
                if self.use_rk2_for_flow:
                    ...
                    if i < self.number_of_time_points - 2:
                        initial_shape = (
                            landmark_points[i + 1].clone().detach().requires_grad_(True)
                        )
                        deformed_volume = self._volume(tensor=initial_shape)
                        deformed_volume.backward()
                        grad_star = initial_shape.grad.detach()
                        d_pos_new = self.kernel.convolve(
                            initial_shape, initial_shape, grad_star
                        )
    ...
    def pole_ladder_transport(self, initial_shoot, initial_time_point=0):
        # Step sizes ---------------------------------------------------------------------------------------------------
        h = 1.0 / self.number_of_time_points
        shoot = initial_shoot

        for i in range(initial_time_point, self.number_of_time_points):
            mom = self.rk4_inverse(
                self.shoot_kernel, self.control_points_t[i], shoot, h
            )
            shoot = self.rk4_step(
                self.shoot_kernel, self.control_points_t[i], -mom, h, return_mom=False
            )
    ...
    def rk4_inverse(cls, kernel, x, y, h):
        ...
        def loss_and_grad(mom):
            ...
            loss = torch.sum(
                (shoot.contiguous().view(-1) - y.contiguous().view(-1)) ** 2
            )
            loss.backward()
    ...
    def _volume(self, tensor):
        ...
        shape = torch.stack(
            [tensor[triangles[:, 0]], tensor[triangles[:, 1]], tensor[triangles[:, 2]]],
            dim=1,
        )
    ...
    def write_control_points_and_momenta_flow(self, name):
        ...
        assert len(self.control_points_t) == len(
            self.momenta_t
